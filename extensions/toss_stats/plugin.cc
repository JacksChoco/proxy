...
#include "extensions/toss_stats/plugin.h"
...
void map_request(IstioDimensions& instance,
  ...
  // instance[request_ipath] = request.request_url_path;
  // instance[request_ipath] = std::regex_replace (request.request_url_path, std::regex("on\\/[a-zA-Z0-9]{8,8}|\\/[a-zA-Z0-9\\=\\_\\,\\:\\.\\-]{12,}|\\/[0-9\\,\\.]{1,}|\\/T[0-9]{1,}"), "/integer");
  // instance[request_ipath] = std::regex_replace (request.request_url_path, std::regex("on\\/[a-zA-Z0-9]{8,8}|\\/[0-9]{1,}|\\/[a-zA-Z0-9\\=\\-_]{12,}|\\/T[0-9]{1,}"), "/{integer}");
...
bool PluginRootContext::initializeDimensions(const json& j) {
  ...
  toss_stats_ = std::vector<StatGen>();
  ...
  for (const auto& factory_it : factories) {
    ...
    toss_stats_.emplace_back(stat_prefix, factory_it.second, tags, indexes,
...
bool PluginRootContext::report(::Wasm::Common::RequestInfo& request_info,
  ...
  auto toss_stats_it = metrics_.find(istio_dimensions_);
  if (toss_stats_it != metrics_.end()) {
    for (auto& stat : toss_stats_it->second) {
  ...
  std::vector<SimpleStat> toss_stats;
  for (auto& statgen : toss_stats_) {
...
RegisterNullVmPluginFactory register_toss_stats_filter("envoy.wasm.toss_stats", []() {
