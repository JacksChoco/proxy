...
class PluginRootContext : public RootContext {
 public:
  PluginRootContext(uint32_t id, std::string_view root_id, bool is_outbound)
    ...
    cache_hits_ = cache_count.resolve("toss_stats_filter", "hit");
    cache_misses_ = cache_count.resolve("toss_stats_filter", "miss");
 ...
 private:
  ...
  Map<uint32_t, std::shared_ptr<::Wasm::Common::RequestInfo>>
      tcp_request_queue_;
  // Peer toss_stats to be generated for a dimensioned metrics set.
  std::vector<StatGen> toss_stats_;
...
static RegisterContextFactory register_TossStatsOutbound(
    CONTEXT_FACTORY(TossStats::PluginContext),
    ROOT_FACTORY(TossStats::PluginRootContextOutbound), "stats_outbound");

static RegisterContextFactory register_TossStatsInbound(
    CONTEXT_FACTORY(TossStats::PluginContext),
    ROOT_FACTORY(TossStats::PluginRootContextInbound), "stats_inbound");
